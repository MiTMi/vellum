...
The Vite build has **two HTML entries** (`rollupOptions.input` in
...
6 `.feature` cards, 4 `.deep .row`s, `#features/#sync/#publish` anchors,
`[data-cta]` hrefs — so structural edits must update
`scripts/e2e-landing.mjs` in the same commit. `assets/hero.png` is currently
...
so each build evicts the last). `registerSW()` is called from both entries and
no-ops unless `import.meta.env.PROD` on an http(s) origin outside Electron.
